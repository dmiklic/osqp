...
csc* lpv_a_mpc_to_osqp_A(csc *Ad[], const csc* Bd, csc *ExtraIneq, c_int N)
{
  ...
  csc *Aineq = csc_eye((N+1)*nx + N*nu, (N+1)*nx + N*nu, 0);
  if (ExtraIneq != OSQP_NULL)
  {
    csc *tmp = Aineq;
    Aineq = csc_vstack(2, Aineq, ExtraIneq);
    csc_spfree(tmp);
  }
  A = csc_vstack(2, Aeq, Aineq);
                           ...
                           c_int n_extra)
{
  ...
  c_float* bineq = vec_cat((N+1)*nx, bineq_x_xN, N*nu, u_bound);
  ...
  if (extra_bound != OSQP_NULL)
  {
    c_float* tmp = bound;
    bound = vec_cat(2*(N+1)*nx+N*nu, bound, n_extra, extra_bound);
    c_free(tmp);
  }
  c_free(beq);
  c_free(bineq);
